...
-(void)setSelectedPhoneNO:(NSString *)selectedPhoneNO{

    _selectedPhoneNO = [self strHandle:selectedPhoneNO];
}



-(NSString *)strHandle:(NSString *)str{
    
    NSMutableString *strM = [NSMutableString stringWithString:str];
    
    [strM replaceOccurrencesOfString:@"-" withString:@"" options:NSLiteralSearch  range:NSMakeRange(0, str.length-1)];

    return [strM copy];
}

-(NSString *)showAllPhoneNO{
    
    if(self.phone.count == 0) return @"空";
    
    if(self.phone.count == 1) return [self takeOutPhoneNo:self.phone.firstObject];
    
    return [self phoneNoStringWithArray:self.phone];
}


-(NSString *)takeOutPhoneNo:(id)obj{
    return [self strHandle:((NSDictionary *)obj).allValues.firstObject];
}

-(NSString *)phoneNoStringWithArray:(NSArray *)arr{
    
    NSMutableString *strM = [NSMutableString string];
    
    [arr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        
        [strM appendFormat:@"%@,",[self takeOutPhoneNo:obj]];
    }];
